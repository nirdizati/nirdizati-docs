...
\begin{figure}[t!]%[H]
	\centering
	 \includegraphics[width=0.8\textwidth]{img/nirdizati-frontend}
...
\begin{figure}[b!]%[H]
	\centering
	 \includegraphics[width=0.8\textwidth]{img/nirdizati-training-architecture-rev}
	%\includegraphics[width=0.7\textwidth]{img/nirdizati-training}
...
Nirdizati Runtime takes a stream of events produced by an organization’s information systems, transforms it into a stream of predictions, and visualizes those predictions in a web-based dashboard.
The transformation is implemented using the dataflow pipeline in \figurename~\ref{fig:dfd_0}, in which the ``predictor'' components are the predictive models from Nirdizati Training.
The pipeline is built atop the the open-source Apache Kafka stream processing platform; the ``topic'' components are network-accessible queues of JSON messages with publisher/subscriber support. This allows the computationally intense work of the predictors to be distributed across a cluster of networked computers, providing scalability and fault-tolerance.
The ``collator'' component accumulates the sequence of events-to-date for each case, such that (for intra-case predictive monitoring) the prediction is a stateless function of only of the (static) trained predictive model and this case history.  This statelessness is what allows the predictors to be freely duplicated and distributed.  The ``joiner'' component composes the original event with the various predictions, ready for display on the dashboard.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{img/nirdizati-runtime}
